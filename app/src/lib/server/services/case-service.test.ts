...
function setup() {
	...
	const runner = { run: vi.fn(async () => ({ outcome: 'finished' as const, summary: '完成', turns: 1, revision: 0 })) };
...
describe('case service', () => {
	it('redacts sensitive input before persistence', () => {
		...
		const result = service.createCase({
			...
			evidence: [{ kind: 'email', content: '发送到 a@example.com', sourceLabel: '通知', occurredAt: null }]
		});
	...
	it('appends evidence, runs the same case, and hides internal configuration', async () => {
		const { repository, runner, service } = setup();
		const created = service.createCase({ title: '入职', goal: '完成手续', confusion: '不知道找谁' });
		const result = await service.appendEvidenceAndRun(created.case.id, {
			kind: 'message', content: '找部门 HR', sourceLabel: '同事', occurredAt: null
		});
		expect(result.case.evidence).toHaveLength(1);
		expect(runner.run).toHaveBeenCalledWith(created.case.id);
		expect(JSON.stringify(service.getCase(created.case.id))).not.toMatch(/API_KEY|ACCESS_SECRET|messages/i);
		repository.close();
	});
	...
	it('returns health flags and rejects unknown case IDs', () => {
		...
		expect(service.health()).toEqual({
			version: 'test', databaseReady: true, modelConfigured: true, zhihuConfigured: true
		});
